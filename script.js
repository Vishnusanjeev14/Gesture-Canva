...
class GestureCanvas {
    ...
    setupEventListeners() {
        ...
        this.colorPicker.addEventListener('input', (e) => {
            ...
            // Optional: update the label border to the new color
            document.querySelector('.color-label').style.borderColor = e.target.value;
        });

        this.sizeSlider.addEventListener('input', (e) => {
            this.brushSize = parseInt(e.target.value, 10);
            this.sizeValueDisplay.textContent = e.target.value;
        });
        // --- END OF ADDITION ---
    }
    ...
    updateCurrentTool(gestureResult) {
        if (gestureResult.tool !== this.currentTool) {
            ...
            // Update drawing canvas cursor
            this.drawingCanvas.className = 'drawing-canvas';
            if (gestureResult.tool === 'pan') {
                this.drawingCanvas.classList.add('panning');
            }

            // --- ADDED LOGIC FOR DYNAMIC BORDERS ---
            const toolClass = gestureResult.tool + '-mode'; // e.g., 'draw-mode', 'pan-mode'
            
            // Update Gesture Panel
            this.gesturePanel.className = 'gesture-panel ' + toolClass;
            
            // Update Video Container (and preserve hidden class if applied)
            const isHidden = this.videoContainer.classList.contains('hidden');
            this.videoContainer.className = 'video-container ' + toolClass;
            if (isHidden) {
    ...
    drawLiveFeedback() {
        ...
        let size = 8;
        let color = '#8E8E93'; // Default pointer color

        if (this.currentTool === 'draw') {
            size = (this.brushSize / 2) * this.zoom;
            color = '#007AFF'; // --- CHANGED: Hard-coded blue for the cursor ---

        } else if (this.currentTool === 'erase') {
            size = (this.eraserSize / 2) * this.zoom;
            color = '#FF3B30'; // Red for eraser
        }

        // Only draw the circle for these tools
    ...
    startDrawing(worldPos, type) {
        this.isDrawing = true;
        // --- UPDATED for color/size controls (this.brushColor is still used for the line) ---
